...
var RecordsServiceClient = function () {
  function RecordsServiceClient(rs, tracer, meterRegistry) {
    this._rs = rs;
    this._tracer = tracer;
    this.recordsTrace = rsocket_rpc_tracing.trace(tracer, "RecordsService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.RecordsService"}, {"method": "records"}, {"rsocket.rpc.role": "client"});
    this.recordsMetrics = rsocket_rpc_metrics.timed(meterRegistry, "RecordsService", {"service": "io.rsocket.reactivesummit.demo.RecordsService"}, {"method": "records"}, {"role": "client"});
  }
  RecordsServiceClient.prototype.records = function records(message, metadata) {
    ...
    return this.recordsMetrics(
      this.recordsTrace(map)(new rsocket_flowable.Flowable(subscriber => {
        ...
        var metadataBuf = rsocket_rpc_frames.encodeMetadata('io.rsocket.reactivesummit.demo.RecordsService', 'records', tracingMetadata, metadata || Buffer.alloc(0));
...
var TournamentServiceClient = function () {
  function TournamentServiceClient(rs, tracer, meterRegistry) {
    this._rs = rs;
    this._tracer = tracer;
    this.tournamentTrace = rsocket_rpc_tracing.trace(tracer, "TournamentService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.TournamentService"}, {"method": "tournament"}, {"rsocket.rpc.role": "client"});
    this.tournamentMetrics = rsocket_rpc_metrics.timed(meterRegistry, "TournamentService", {"service": "io.rsocket.reactivesummit.demo.TournamentService"}, {"method": "tournament"}, {"role": "client"});
  }
  TournamentServiceClient.prototype.tournament = function tournament(message, metadata) {
    ...
    return this.tournamentMetrics(
      this.tournamentTrace(map)(new rsocket_flowable.Flowable(subscriber => {
        var dataBuf = Buffer.from(message.serializeBinary());
        var tracingMetadata = rsocket_rpc_tracing.mapToBuffer(map);
        var metadataBuf = rsocket_rpc_frames.encodeMetadata('io.rsocket.reactivesummit.demo.TournamentService', 'tournament', tracingMetadata, metadata || Buffer.alloc(0));
          this._rs.requestStream({
            data: dataBuf,
            metadata: metadataBuf
          }).map(function (payload) {
...
var RankingServiceClient = function () {
  function RankingServiceClient(rs, tracer, meterRegistry) {
    this._rs = rs;
    this._tracer = tracer;
    this.rankTrace = rsocket_rpc_tracing.traceSingle(tracer, "RankingService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.RankingService"}, {"method": "rank"}, {"rsocket.rpc.role": "client"});
    this.rankMetrics = rsocket_rpc_metrics.timedSingle(meterRegistry, "RankingService", {"service": "io.rsocket.reactivesummit.demo.RankingService"}, {"method": "rank"}, {"role": "client"});
  }
  RankingServiceClient.prototype.rank = function rank(message, metadata) {
    ...
    return this.rankMetrics(
      this.rankTrace(map)(new rsocket_flowable.Single(subscriber => {
        var dataBuf = Buffer.from(message.serializeBinary());
        var tracingMetadata = rsocket_rpc_tracing.mapToBuffer(map);
        var metadataBuf = rsocket_rpc_frames.encodeMetadata('io.rsocket.reactivesummit.demo.RankingService', 'rank', tracingMetadata, metadata || Buffer.alloc(0));
...
var RecordsServiceServer = function () {
  function RecordsServiceServer(service, tracer, meterRegistry) {
    ...
    this._tracer = tracer;
    this.recordsTrace = rsocket_rpc_tracing.traceAsChild(tracer, "RecordsService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.RecordsService"}, {"method": "records"}, {"rsocket.rpc.role": "server"});
    this.recordsMetrics = rsocket_rpc_metrics.timed(meterRegistry, "RecordsService", {"service": "io.rsocket.reactivesummit.demo.RecordsService"}, {"method": "records"}, {"role": "server"});
...
var TournamentServiceServer = function () {
  function TournamentServiceServer(service, tracer, meterRegistry) {
    this._service = service;
    this._tracer = tracer;
    this.tournamentTrace = rsocket_rpc_tracing.traceAsChild(tracer, "TournamentService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.TournamentService"}, {"method": "tournament"}, {"rsocket.rpc.role": "server"});
    this.tournamentMetrics = rsocket_rpc_metrics.timed(meterRegistry, "TournamentService", {"service": "io.rsocket.reactivesummit.demo.TournamentService"}, {"method": "tournament"}, {"role": "server"});
    this._channelSwitch = (payload, restOfMessages) => {
      if (payload.metadata == null) {
        return rsocket_flowable.Flowable.error(new Error('metadata is empty'));
      }
...
var RankingServiceServer = function () {
  function RankingServiceServer(service, tracer, meterRegistry) {
    this._service = service;
    this._tracer = tracer;
    this.rankTrace = rsocket_rpc_tracing.traceSingleAsChild(tracer, "RankingService", {"rsocket.rpc.service": "io.rsocket.reactivesummit.demo.RankingService"}, {"method": "rank"}, {"rsocket.rpc.role": "server"});
    this.rankMetrics = rsocket_rpc_metrics.timedSingle(meterRegistry, "RankingService", {"service": "io.rsocket.reactivesummit.demo.RankingService"}, {"method": "rank"}, {"role": "server"});
    this._channelSwitch = (payload, restOfMessages) => {
      if (payload.metadata == null) {
        return rsocket_flowable.Flowable.error(new Error('metadata is empty'));
      }
